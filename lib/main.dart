            ...
            return ListView.builder(
                ...
                itemBuilder: (BuildContext context, int index) {
                  var user = User(
                    ...
                    courses:
                        snapshot.data!.docs[index].data().containsKey("courses")
                            ? snapshot.data!.docs[index].get("courses")
                            : [],
                    ...
                    salary: int.parse(snapshot.data!.docs[index].get("salary"),),
                    sex: snapshot.data!.docs[index].get("sex"),
                    universitys: snapshot.data!.docs[index]
                            .data()
                            .containsKey("universitys")
                        ? snapshot.data!.docs[index].get("universitys")
                        : [],
                    aquiredAbilities:
                        snapshot.data!.docs[index].get("aquiredAbilities"),
                    desiredJob: snapshot.data!.docs[index].get("desiredJob"),
                    educations: snapshot.data!.docs[index]
                            .data()
                            .containsKey("educations")
                        ? snapshot.data!.docs[index].get("educations")
                        : [],
                    expirience: snapshot.data!.docs[index]
                            .data()
                            .containsKey("expirience")
                        ? snapshot.data!.docs[index].get("expirience")
                        : [],
                  );
                  if (user.fio.contains(FilterCustomState.fiotext) &&
                      user.age > FilterCustomState.ageFromtext &&
                      user.age < FilterCustomState.ageUntiltext
                      && user.salary > FilterCustomState.salaryFromtext &&
                      user.salary < FilterCustomState.salaryUntiltext
                      ) {}
                  return Card(
                      child: ListTile(
                          title: Text(user.fio),
